  ...
  return (
    <div>
      <TextBg
        ...
        className="!text-lg lg:!text-4xl font-bold text-black my-8 lg:my-16 text-center"
      />
      ...
      {accordionArr.map((accordion, index) => (
        ...
        >
          <div className={`transition-all py-2 mx-auto  overflow-hidden`}>
            <div
              className="w-full px-2 lg:px-8 border-b border-black flex justify-between items-center cursor-pointer"
              onClick={() => handleActiveState(index)}
            >
              <h2 className="text-md w-[80%] lg:w-full lg:text-xl font-medium">
                {accordion.bgText}
              </h2>
              {activeAccordion === index ? <ArrowUp2 /> : <ArrowDown2 />}
            </div>
            {activeAccordion === index && (
              <div className="w-full px-2 lg:px-8 rounded-bl-[10px] rounded-br-[10px]  border border-black bg-white py-2">
                <TextSm
                  content={accordion.smText}
                  className="text-grey-100 font-medium text-md lg:!text-lg"
                />
              </div>
            )}
          </div>
      ...
      <motion.div
        ...
        whileInView="show"
        ref={scrollRef}
        className="w-full md:mt-28 mt-12 "
      >
